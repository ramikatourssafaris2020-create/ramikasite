// Bus grid navigation logic
function scrollBusGrid(direction) {
    const grid = document.getElementById('busGridRow');
    const itemWidth = grid.children[0].offsetWidth;
    grid.parentElement.scrollBy({left: direction * itemWidth, behavior: 'smooth'});
    setTimeout(updateBusGridDots, 400);
}

function scrollBusGridTo(index) {
    const grid = document.getElementById('busGridRow');
    const itemWidth = grid.children[0].offsetWidth;
    grid.parentElement.scrollTo({left: index * itemWidth, behavior: 'smooth'});
    setTimeout(updateBusGridDots, 400);
}

function updateBusGridDots() {
    const grid = document.getElementById('busGridRow');
    const scrollLeft = grid.parentElement.scrollLeft;
    const itemWidth = grid.children[0].offsetWidth;
    const index = Math.round(scrollLeft / itemWidth);
    const dots = document.querySelectorAll('#busGridDots .bus-dot');
    dots.forEach((dot, i) => {
...
document.addEventListener('DOMContentLoaded', function() {
    const grid = document.getElementById('busGridRow');
    if (grid) {
        grid.parentElement.addEventListener('scroll', updateBusGridDots);
        updateBusGridDots();
    }
});
